Shader "CSCI4160U/CameraBloodEffect" {
	Properties{
		...
		SubShader{
		// No culling or depth (standard settings for image effect shader)
		...
		v2f vert(appdata v) {
			v2f o;
			//o.vertex = mul(UNITY_MATRIX_MVP, v.vertex); // old way
			o.vertex = UnityObjectToClipPos(v.vertex);    // new way
			o.uv = v.uv;
			return o;
		}
